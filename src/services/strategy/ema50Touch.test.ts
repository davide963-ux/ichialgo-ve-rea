...
function bar(prev: Candle, { low, high, close }: { low: number; high: number; close: number }): Candle {
  return { time: prev.time + STEP_S, open: prev.close, high, low, close, volume: null, complete: true };
}
...
describe('analyseEma50Touch', () => {
  it('reports not-enough-bars below the warm-up window', () => {
    const a = analyseEma50Touch(candles(uptrend(40)), SYMBOL, '30M');
  ...
  it('finds no touch while price runs away from the EMA', () => {
    const a = analyseEma50Touch(candles(uptrend()), SYMBOL, '30M');
  ...
  it('signals once when a pullback reaches the EMA from above', () => {
    ...
    const a = analyseEma50Touch(candles(closes), SYMBOL, '30M');
    ...
    expect(s.id).toBe(`ema50-touch|EUR/USD|30M|${a.signals[0]!.barTime}`);
  });
  ...
  it('marks a touch on the still-forming bar as pending', () => {
    ...
    expect(analyseEma50Touch(bars, SYMBOL, '30M').signals.at(-1)?.outcome).toBe('pending');
  });
  ...
  it('reads a rally into a falling EMA as a short', () => {
    ...
    const a = analyseEma50Touch(candles([...downtrend, ema]), SYMBOL, '30M');
  ...
  it('widens the tolerance with volatility', () => {
    const closes = uptrend();
    const ema = emaOfCloses(candles(closes), EMA50_TOUCH.period).at(-1)!;
    const calm = analyseEma50Touch(candles([...closes, ema], 2), SYMBOL, '30M').signals.at(-1)!;
    const wild = analyseEma50Touch(candles([...closes, ema], 40), SYMBOL, '30M').signals.at(-1)!;
